...
output:
  ...
  html_document: null
  theme: default
subtitle: GxEMM gtype=iid, etype=hom VS gtype=iid, etype=iid
keep_md: yes
urlcolor: blue
---
...
* Model 1: gtype=iid, etype=hom
* Model 2: gtype=iid, etype=iid
...
models <- c("slopeGandE_add_12", "gxemm_iid", "gxemm_giid_eiid")
...
# Simulation: $h^2_{cov} + \sigma^2_{cov}$

```{r sim1, cache = T}
resFile <- '../results/sim_gxemm_iidVShom.RData'
...
if(!file.exists(resFile)){
  ...
  SimRes <- data.frame(grid, runtime_slopeGE = NA, runtime_ehom_gxemm = NA, runtime_eiid_gxemm = NA, 
                       h2.add.t2_slopeMod_GE = NA, 
                       h2.cov_slopeMod_GE = NA, 
                       sigma.cov_slopeMod_GE = NA,
                       gxemm_ehom_h2.hom = NA,
                       gxemm_ehom_h2.het = NA,
  ...
  for(i in 1:nrow(grid)){
    ...
    SimRes$runtime_ehom_gxemm[i] <- simdat.modFit$runtime[simdat.modFit$model == 'gxemm_iid']
    ...
    SimRes$gxemm_ehom_h2.hom[i] <- simdat.modFit$h2.hom_gxemm[simdat.modFit$model == 'gxemm_iid']
    SimRes$gxemm_ehom_h2.het[i] <- simdat.modFit$h2.het_gxemm[simdat.modFit$model == 'gxemm_iid']
...
```{r plot_h2cov_ecov_iidVShom, echo = F, cache = T}
...
# Runtime
```{r plot_runtime_iidVShom, echo = F, cache = T}
boxplot(SimRes$runtime_slopeGE, SimRes$runtime_ehom_gxemm, SimRes$runtime_eiid_gxemm, xaxt = 'n', ylab = '')
axis(side = 1, at = 1:3, labels = c('Random Slope GE', 'GxEMM ehom', 'GxEMM eiid'), cex.axis = 1)
