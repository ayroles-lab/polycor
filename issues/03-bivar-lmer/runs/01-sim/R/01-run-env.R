...
out <- lapply(seq(nrow(grid)), function(i)
{
  ...
  dat <- sim_bivar_env(e1, e2, rhoe, n)
  bdat <- gather(dat, tname, tvalue, y1, y2) 

  mod <- relmer(tvalue ~ tname - 1 + (0 + tname | id), data = bdat, weights = rep(1e10, nrow(bdat)), calc.derivs = FALSE)
  vf <- VarCorr(mod) %>% as.data.frame
  rhoe_hat <- subset(vf, var1 == "tnamey1" & var2 == "tnamey2", "sdcor", drop = TRUE)
  
  data_frame(sim = sim, n = n, rhoe = rhoe, rhoe_hat = rhoe_hat, e1 = e1, e2 = e2)
})

tab <- bind_rows(out)

### save
saveRDS(tab, "env_rhoe.rds")

# plot
# ggplot(tab, aes(rhoe, rhoe_hat, group = as.factor(rhoe))) + geom_boxplot() + facet_wrap(~ n, nrow = 1) + geom_abline(linetype = 3) + coord_equal() + theme_minimal()
