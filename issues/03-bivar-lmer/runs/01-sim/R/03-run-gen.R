...
out <- lapply(seq(nrow(grid)), function(i)
{
  ...
  kin <- sim_kin(h1, h2, rhog, rhoe, n)
  dat <- sim_bivar(h1, h2, rhog, rhoe, n)
  bdat <- gather(dat, tname, tvalue, y1, y2) %>%
    mutate(tindex = (as.integer(as.factor(tname)) - 1))

  mod <- relmer(tvalue ~ tname - 1 + (0 + tname | id) + (0 + tname | rid), data = bdat, weights = rep(1e10, nrow(bdat)), relmat = list(id = kin), calc.derivs = FALSE)
  vf <- VarCorr(mod) %>% as.data.frame %>% filter(grp == "id" & var1 == "tnamey1" & var2 == "tnamey2")
  rhog_hat <- vf$sdcor
  
  data_frame(sim = sim, n = n, rhog = rhog, rhog_hat = rhog_hat, h1 = h1, h2 = h2, rhoe = rhoe)
})

tab <- bind_rows(out)

### save
saveRDS(tab, "gen_rhog.rds")
