...
out <- lapply(seq(nrow(grid)), function(i)
{
  ...
  #mod1 <- relmer(tvalue ~ tname - 1 + (1 + tindex || id) + (1 + tname | obs), data = bdat, weights = rep(1e10, nrow(bdat)), calc.derivs = FALSE)

  data_frame(sim = sim, n = n/nrep, nrep = nrep, rhoe = rhoe, rhoe_hat = rhoe_hat, e1 = e1, e2 = e2)
})
