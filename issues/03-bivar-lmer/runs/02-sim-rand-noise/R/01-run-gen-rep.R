  ...
  scale = TRUE, sim_rand_gen = FALSE) 
{
  ...
  V_res <- kronecker(Sigma_resid, Diagonal(N))

  if(!sim_rand_gen) {
    y12_gen <- mvrnorm(1, rep(0, 2*N), V_gen)
  }
  
  lapply(seq(1, rep), function(r) {
    y12_res <- mvrnorm(1, rep(0, 2*N), V_res)
...
out <- lapply(seq(nrow(grid)), function(i)
{
  ...
  mod <- relmer(tvalue ~ -1 + rep + tname + (0 + tname | id) + (0 + tname | rid), data = bdat, relmat = list(id = kin), calc.derivs = FALSE, verbose = 2)
